    ...
    if (TucAuthentification::connectedUser() and isset($acces_utilisateur)) {
      ...
      if ($httpRequest->method === 'GET') {
        ...
        if (!isset($httpRequest->get['idGalerie'])) {
          ...
          DB::table('mots_clefs_galeries')->insert(['mot_clef' => "$motclef->mot_clef", 'id_galerie' => "$$httpRequest->get['id']"]);
          Router::executeRoute('galerie');

        } else {

          DB::table('mots_clefs_galeries')->insert(['mot_clef' => "$motclef->mot_clef", 'id_photo' => "$$httpRequest->get['id']"]);
